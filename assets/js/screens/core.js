...
function syncDivLock() {
  ...
  const tip = `Заполните все очки в раунде ${ppc} хотя бы на одном корте (1–${nc}), чтобы открыть`;
  document.querySelectorAll('.pill-div-btn').forEach(p => {
...
async function _switchTabInner(id) {
  ...
  if (id === 'hard' || id === 'advance' || id === 'medium' || id === 'lite') {
    if (!hasRound5Score()) {
      showToast(`🔒 Заполните все очки в раунде ${ppc} хотя бы на одном корте (1–${nc})`);
      activeTabId = prevTabId;
      syncNavActive();
      return;
    }
